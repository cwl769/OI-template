...
template <i64 MOD> class Int {
private:
  int v;

public:
  Int() : v() {}
  Int(const Int& b) { v = b.v; }
  Int(Int&& b) { v = b.v; }
  Int(i64 x) {
    if (x < 0) {
      v = (x % MOD) + MOD;
  ...
  Int operator-(Int b) const {
    Int ans;
    ans.v = v - b.v;
    if (ans.v < 0) ans.v += MOD;
    return ans;
  }
  Int operator*(Int b) const { return 1ll * v * b.v; }
  Int& operator+=(Int b) {
    v += b.v;
    if (v >= MOD) v -= MOD;
    return *this;
...
template <i64 MOD>
std::vector<Int<MOD>> NTT(std::vector<Int<MOD>> a, int len,
                          i64 g) {
  len = (1 << (log2(len - 1) + 1));
  a.resize(len);
...
template <i64 MOD, i64 g> class PolyMod {
private:
public:
  ...
  PolyMod inv(int n) const {
    if (n == 1) { return PolyMod(v[0] ^ (MOD - 2)); }
    PolyMod f0 = this->inv((n + 1) >> 1);
    PolyMod tmp =
      f0 * (PolyMod(2) - (*this * f0).cut(n)).cut(n);
    return tmp.cut(n);
  }
  friend std::ostream& operator<<(std::ostream& stream,
                                  const PolyMod& poly) {
    for (auto x : poly.v) { stream << x.val() << ' '; }
    return stream;
  }
};