	...
	\begin{multicols}{2}
		...
		\begin{spacing}{0.6}
			...
			\section{Data Structure}
				...
				\subsection{珂朵莉树}
					\inputminted{cpp}{DataStructure/odt.cpp}
			


			\section{Math}
				\subsection{实数多项式}
					\inputminted{cpp}{Math/poly.cpp}
				\subsection{模多项式(NTT)}
					\inputminted{cpp}{Math/polyMod.cpp}
				\subsection{插值}
					\input{Math/interp.tex}
					\inputminted{cpp}{Math/interp_lagrange.cpp}
				\subsection{高维前缀和/子集和}
					\inputminted{cpp}{Math/subsetsum.cpp}
				\subsection{容斥原理}
					\input{Math/in-ex-clusion.tex}
				\subsection{万能欧几里德算法}
					\input{Math/euclid.tex}
			...
			\section{Geometry}
				\subsection{整点凸包}
					\inputminted{cpp}{Geometry/geometry.cpp}
